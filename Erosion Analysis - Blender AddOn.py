...
# The "original" surface is usually generated by the user, and represents the expected original surface of the building before being eroded.
# The "original" surface must be subdivided into a large number of tiles, small enough to act as the pixels of an image.
# In this way, after running the script, the "original" surface is colored like an image, to represent the erosion distribution.

# Only faces with Z within "Z Minimum"-"Z Maximum" and with erosion within "Erosion Limit", are processed.
# The erosion value is stored as a gray color in a new Color Attribute ("Erosion"), that is used to colorcode the object surface based on the amount of erosion.
# The colors can be chosen between "Grey Scale" and "Rainbow Scale"
# A transparent color is assigned to the faces where an erosion value cannot be calculated.
...
class SaveDataOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            props.status_label = "%.3f <> %.3f" % (erosion_min, erosion_max)
...
class MyToolPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.prop(props, "status_label", text="Erosion Range:")

class MyProperties(bpy.types.PropertyGroup):
    original_obj: bpy.props.PointerProperty(name="Original Surface", type=bpy.types.Object)
